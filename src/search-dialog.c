...
sd_response_cb (GtkDialog *dialog, gint response_id, AlmanahSearchDialog *search_dialog)
{
	...
	if (search_dialog->sd_cancellable != NULL) {
		g_cancellable_cancel (search_dialog->sd_cancellable);
	}
...
sd_search_progress_cb (AlmanahStorageManager *storage_manager, AlmanahEntry *entry, AlmanahSearchDialog **search_dialog_weak_pointer)
{
	...
	gtk_list_store_append (search_dialog->sd_results_store, &tree_iter);
	gtk_list_store_set (search_dialog->sd_results_store, &tree_iter,
...
sd_search_ready_cb (AlmanahStorageManager *storage_manager, GAsyncResult *res, AlmanahSearchDialog **search_dialog_weak_pointer)
{
	...
	gtk_spinner_stop (search_dialog->sd_search_spinner);
	gtk_widget_hide (GTK_WIDGET (search_dialog->sd_search_spinner));
	gtk_widget_set_sensitive (search_dialog->sd_cancel_button, FALSE);
	gtk_widget_set_sensitive (search_dialog->sd_search_button, TRUE);

	if (error != NULL && g_error_matches (error, G_IO_ERROR, G_IO_ERROR_CANCELLED) == TRUE) {
		gtk_label_set_text (search_dialog->sd_results_label, _ ("Search canceled."));
	} else if (error != NULL) {
		/* Translators: This is an error message wrapper for when searches encounter an error. The placeholder is for an error message. */
		g_autofree gchar *error_message = g_strdup_printf (_ ("Error: %s"), error->message);
		gtk_label_set_text (search_dialog->sd_results_label, error_message);
	} else {
		/* Success! */
		...
		gtk_label_set_text (search_dialog->sd_results_label, results_string);
	}
	...
	g_object_unref (search_dialog->sd_cancellable);
	search_dialog->sd_cancellable = NULL;
}
...
sd_search_button_clicked_cb (GtkButton *self, AlmanahSearchDialog *search_dialog)
{
	...
	gtk_label_set_text (search_dialog->sd_results_label, _ ("Searching…"));
	gtk_widget_show (GTK_WIDGET (search_dialog->sd_search_spinner));
	gtk_spinner_start (search_dialog->sd_search_spinner);
	...
	*search_dialog_weak_pointer = search_dialog;
	...
	almanah_storage_manager_search_entries_async (storage_manager, search_string, search_dialog->sd_cancellable,
...
sd_view_button_clicked_cb (GtkButton *self, AlmanahSearchDialog *search_dialog)
{
	...
	if (gtk_tree_selection_get_selected (search_dialog->sd_results_selection, &model, &iter) == TRUE) {
