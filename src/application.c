...
{
	AlmanahApplicationPrivate *priv = almanah_application_get_instance_private(ALMANAH_APPLICATION(self));
	priv->main_window = NULL;
}
...
action_search_cb (GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
	...
	AlmanahSearchDialog *dialog;

	application = ALMANAH_APPLICATION (user_data);
	priv = almanah_application_get_instance_private(ALMANAH_APPLICATION(application));
	dialog = almanah_search_dialog_new ();
...
action_quit_cb (GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
	...
	priv = almanah_application_get_instance_private(ALMANAH_APPLICATION (user_data));
	main_window = priv->main_window;
...
almanah_application_get_debug (AlmanahApplication *self)
{
	...
	priv = almanah_application_get_instance_private(self);
	return priv->debug;
}
