...
almanah_uri_entry_dialog_get_property (GObject *object, guint property_id, GValue *value, GParamSpec *pspec)
{
	...
	priv = almanah_uri_entry_dialog_get_instance_private (ALMANAH_URI_ENTRY_DIALOG (object));
...
almanah_uri_entry_dialog_run (AlmanahUriEntryDialog *self)
{
	...
	priv = almanah_uri_entry_dialog_get_instance_private(self);
...
/* TODO: if the uri could not be set, we must notify the caller */
void
